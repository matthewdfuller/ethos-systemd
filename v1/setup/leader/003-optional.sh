...
SERVICES_OVERRIDE=$(sudo docker run --rm --privileged --userns host behance/docker-aws-secrets-downloader --table $SECRETS_TABLE --key configs | grep /environment/services)

if [[ ! -z $SERVICES_OVERRIDE ]]; then
	SERVICES=$(sudo docker run --rm --privileged --userns host behance/docker-aws-secrets-downloader --table $SECRETS_TABLE --key configs --name /environment/services)
