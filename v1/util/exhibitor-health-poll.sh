...
ZK_AUTH="`etcdctl -u $ETCDCTL_READ_USER:$ETCDCTL_READ_PASSWORD get /zookeeper/config/username`:`etcdctl -u $ETCDCTL_READ_USER:$ETCDCTL_READ_PASSWORD get /zookeeper/config/password`"
...
CTL_SIZE="`etcdctl -u $ETCDCTL_READ_USER:$ETCDCTL_READ_PASSWORD get /zookeeper/config/ensemble-size`"
